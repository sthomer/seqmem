...
data Level a = Level { unigram :: Unigram a, bigram :: Bigram a, segment :: Segment a}
...
level :: Level a
level = Level { unigram = M.empty, bigram = M.empty, segment = [] }
...
push :: Token a -> State (Level a) ()
push x = state $ \l -> ((), incSegment l x)
